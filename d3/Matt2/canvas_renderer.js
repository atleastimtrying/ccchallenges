...
(function() {
  ...
  */
  ...
  window.CanvasRenderer = (function(_super) {
    ...
    CanvasRenderer.prototype.render = function(physics) {
      ...
      if (this.renderParticles) {
        TWO_PI = Math.PI * 2;
        _ref = physics.particles.reverse();
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          p = _ref[_i];
          this.ctx.beginPath();
          this.ctx.arc(p.pos.x, p.pos.y, p.radius, 0, TWO_PI, false);
          this.ctx.fillStyle = p.colour || 'rgba(255,255,255,0.5)';
          this.ctx.fill();
          if (p.name != null) {
            this.ctx.font = '15px sans-serif';
            this.ctx.textAlign = 'left';
            this.ctx.fillStyle = 'blue';
            this.ctx.fillText(p.name, p.pos.x + p.radius + 5, p.pos.y + 5);
          }
        }
      }
